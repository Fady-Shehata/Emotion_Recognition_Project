import 'package:testapp/modules/Emotions/user.dart';
...
class EmotionsScreen extends StatefulWidget {
  ...
  @override
  _EmotionsScreen createState() => _EmotionsScreen();
}

class _EmotionsScreen extends State<EmotionsScreen> {
  ...
  void initState() {
    ...
    setState(() {
      _foundedEmotions = _emotions;
    });
  }

onSearch(String search) {
    setState(() {
      _foundedEmotions = _emotions.where((user) => user.name.toLowerCase().contains(search)).toList();
    });
  }
  ...
  Widget build(BuildContext context) {
    return SafeArea(
        ...
        child: _foundedEmotions.length > 0 ? ListView.builder(
          itemCount: _foundedEmotions.length,
